...
class MetaMarketingController extends Controller
{
            ...
            case 'pdf':
                ...
                if (function_exists('shell_exec')) {
                    ...
                    if ($bin !== '') {
                        ...
                        if (!empty($out)) return ['content' => $out];
                    }
                }
                // 2) Pure-PHP fallback for cPanel hosts without poppler-utils
                $text = $this->extractPdfTextPurePhp($path);
                if ($text === null) {
                    return ['error' => 'Não foi possível extrair texto deste PDF (provavelmente digitalizado/imagem). Converta para TXT/MD/DOCX e tente novamente.'];
                }
                return ['content' => $text];
    ...
    /**
     * Pure-PHP PDF text extraction for cPanel hosts without pdftotext.
     * Handles uncompressed and FlateDecoded text streams. Works for PDFs
     * generated by software (Word, LibreOffice, web). Returns null if it
     * cannot extract anything (e.g. scanned/image-only PDFs).
     */
    private function extractPdfTextPurePhp(string $path): ?string
    {
        $raw = @file_get_contents($path);
        if ($raw === false || strncmp($raw, '%PDF-', 5) !== 0) return null;

        $text = '';
        // Find all stream...endstream blocks
        $offset = 0;
        while (($a = strpos($raw, 'stream', $offset)) !== false) {
            $b = strpos($raw, 'endstream', $a);
            if ($b === false) break;
            $startPos = $a + 6;
            // Skip newline after "stream"
            if (isset($raw[$startPos]) && $raw[$startPos] === "\r") $startPos++;
            if (isset($raw[$startPos]) && $raw[$startPos] === "\n") $startPos++;

            $data = substr($raw, $startPos, $b - $startPos);
            $offset = $b + 9;

            // Try FlateDecode (zlib) — most common
            $decoded = null;
            ...
            // Extract text from PDF text-showing operators: (text)Tj, [(text)...]TJ
            if (preg_match_all('/\((?:\\\\.|[^\\\\()])*\)\s*Tj/s', $decoded, $m1)) {
                foreach ($m1[0] as $hit) {
                    $inner = substr($hit, 1, strpos($hit, ')Tj') - 1);
                    $text .= $this->decodePdfString($inner) . ' ';
                }
            }
            if (preg_match_all('/\[(.*?)\]\s*TJ/s', $decoded, $m2)) {
                foreach ($m2[1] as $arr) {
                    if (preg_match_all('/\((?:\\\\.|[^\\\\()])*\)/s', $arr, $m3)) {
                        foreach ($m3[0] as $piece) {
    ...
    private function decodePdfString(string $s): string
    {
        ...
        // Try to convert from common PDF encodings to UTF-8
        if (mb_check_encoding($s, 'UTF-8') === false) {
            $s = @mb_convert_encoding($s, 'UTF-8', 'Windows-1252,ISO-8859-1') ?: $s;
        }
        return $s;
    }
