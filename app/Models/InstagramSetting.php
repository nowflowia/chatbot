<?php

namespace App\Models;

use Core\Model;
use Core\Database;

class InstagramSetting extends Model
{
    protected static string $table = 'instagram_settings';

    public static function getActive(): ?array
    {
        return Database::getInstance()->selectOne(
            "SELECT * FROM instagram_settings ORDER BY id ASC LIMIT 1"
        );
    }

    public static function saveSettings(array $data): array
    {
        $db       = Database::getInstance();
        $existing = static::getActive();

        $payload = [
            'app_id'               => $data['app_id']               ?? null,
            'app_secret'           => $data['app_secret']           ?? null,
            'access_token'         => $data['access_token']         ?? null,
            'instagram_account_id' => $data['instagram_account_id'] ?? null,
            'page_id'              => $data['page_id']              ?? null,
            'webhook_verify_token' => $data['webhook_verify_token'] ?? null,
            'api_version'          => $data['api_version']          ?? 'v21.0',
            'updated_at'           => now(),
        ];

        if ($existing) {
            $sets = implode(', ', array_map(fn($k) => "`{$k}` = ?", array_keys($payload)));
            $db->update(
                "UPDATE instagram_settings SET {$sets} WHERE id = ?",
                [...array_values($payload), $existing['id']]
            );
            return array_merge($existing, $payload);
        }

        $payload['name']       = 'Principal';
        $payload['status']     = 'inactive';
        $payload['created_at'] = now();
        $id = $db->insert(
            "INSERT INTO instagram_settings (" . implode(',', array_map(fn($k) => "`{$k}`", array_keys($payload))) . ")
             VALUES (" . implode(',', array_fill(0, count($payload), '?')) . ")",
            array_values($payload)
        );
        return static::find((int)$id) ?? $payload;
    }

    public static function updateStatus(int $id, string $status, ?string $error = null): void
    {
        Database::getInstance()->update(
            "UPDATE instagram_settings SET status = ?, last_error = ?, last_tested_at = NOW(), updated_at = NOW() WHERE id = ?",
            [$status, $error, $id]
        );
    }
}
